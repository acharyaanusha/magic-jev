...
- [ ] **github.ts.** Base `https://api.github.com`, headers `Authorization: Bearer <token>`, `Accept: application/vnd.github+json`, `X-GitHub-Api-Version: 2022-11-28`. `fetchRawPr` gets the pull first, then in parallel: files and reviews (follow pagination with `per_page=100` and `page=N` until a short page, capped at 10 pages), the combined status and the check runs for `pull.head.sha` (`per_page=100`). A non-2xx response throws `GitHubError` with the status. `searchReviewRequests` calls `/search/issues?q=` with `is:open is:pr review-requested:@me archived:false`, URL-encoded, `per_page=50`.
